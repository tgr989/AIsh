...
old_conf="$(mktemp)"
new_conf="$(mktemp)"
trap 'rm -f "$old_conf" "$new_conf"' EXIT
...
expect_ok render_snat_candidate "$old_conf" "$new_conf"
...
grep -Fq 'snat to 192.0.2.10' "$new_conf" || fail 'listen IPv4 was not used for SNAT'
