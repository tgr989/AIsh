...
render_conf_file() {
    ...
    if ! {
        ...
        for r in "${RULES[@]+"${RULES[@]}"}"; do
            ...
            printf '        %sct status dnat ct original ip daddr %s meta l4proto %s ct original proto-dst %s %sip daddr %s %s dport %s masquerade\n' \
...
runtime_rules_match_loaded_config() {
    ...
    for r in "${RULES[@]+"${RULES[@]}"}"; do
        ...
        expected["${CHAIN_POSTROUTING}|${iif_expr}ct status dnat ct original ip daddr ${listen_ip} meta l4proto ${proto} ct original proto-dst ${lport} ${original_source_expr}ip daddr ${dip} ${proto} dport ${dport} masquerade"]=1
    done
...
常用命令：
  ...
  grep -E 'RULE:|RULE-MASQ:' ${CONF_FILE}
  nft -c -f ${CONF_FILE}
...
add_rule_interactive() {
    ...
    echo
    info "将添加规则（DNAT + MASQUERADE）:"
    for p in "${protos_to_add[@]}"; do
        echo "  ${p} ${listen_ip}:${lport} -> ${dip}:${dport} (iif=${iif}, source=${source_cidr})"
    done
